...
yrbss_state_tidy_additional <- 
  yrbss_state_tidy_additional %>% 
  mutate(
    drug_use_freq = factor(drug_use_freq, levels = c("No Drug Use", "Light Dose", "Heavy Dose")),
    grade = factor(grade, levels = c("9th Grade", "10th Grade", "11th Grade", "12th Grade")),
    sex = factor(sex, levels = c("female", "male")),
    race4 = factor(race4, levels = c("White","Black or African American","Hispanic/Latino","All other Races")),
    drunk_driving = factor(drunk_driving, levels = c("Yes", "No")),
    text_driving = factor(text_driving, levels = c("Yes", "No")),
    carrying_weapon = factor(carrying_weapon, levels = c("Yes", "No")),
    suicide_attempt = factor(suicide_attempt, levels = c("Yes", "No")),
    quit_smoke = factor(quit_smoke, levels = c("Never Smoke", "Yes", "No")),
    physical_fight = factor(physical_fight, levels = c("Yes", "No")),
    early_sex = factor(early_sex, levels = c("Yes", "No")),
    heavy_screen_use = factor(heavy_screen_use, levels = c("Yes", "No")),
    enough_sleep = factor(enough_sleep, levels = c("Yes", "No")),
    smoking_status = factor(smoking_status, levels = c("Never Smoker", "Light Smoker", "Heavy Smoker")),
    binge_drinking = factor(binge_drinking, levels = c("No Binge Drinking", "Light Binge Drinking", "Heavy Binge Drinking")),
    seat_belt = factor(seat_belt, levels = c("Never or Rarely", "Sometimes", "Most of the time or Always")),
    grades_school = factor(grades_school, levels = c("Mostly A's", "Mostly B's", "Mostly C's", "Mostly Below C's", "None of these grades", "Not sure")),
  )
```


This data analysis uses the resulted data set cleaned by the processing described in data analysis part of [Data]("Data_Description.html"). According to our motivation, we are interested the relationship between drug use and youth behaviors. In this analysis, we will explore the association between drug use status and each health behavior variables.
...
In order to test whether there is association between drug use and BMI, which is a continuous variable, we perform a ANOVA test to test the hypothesis that
$$
H_0：\text{the mean BMI of students are same across all drug use status}
$$
$$
H_1: \text{At least two drug use status groups have different mean BMI}
$$
...
From Table 7, we can see that all p-value are significantly small and the effect size are around 0.4, suggesting that the association between drug use status and physical fight is strong. The effect size over each stratum is very close to each other, so the interaction effect is negligible. 
...
ggplot(yrbss_state_tidy_additional, 
  ...
  labs(title="smoke start age",x="drug use frequency")
```
...
ggplot(yrbss_state_tidy_additional, 
  ...
  theme(
    axis.text.x = element_text(angle = 45, vjust = 0.5, hjust = 1),
    legend.position = "none") +
  labs(title="alcohol start age",x="drug use frequency")
...
density_plot=
  ...
  geom_vline(data=BMI, aes(xintercept=mean(bmi), color=sex),
             linetype="dashed") + 
  facet_grid(. ~ drug_use_freq) +
  labs(title = "density plot for BMI",x="drug use frequency")
