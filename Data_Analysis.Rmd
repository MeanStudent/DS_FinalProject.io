...
ggplot(yrbss_state_tidy_additional, 
  ...
  labs(title="smoke start age",xlab="drug use frequency"))
```
...
ggplot(yrbss_state_tidy_additional, 
  ...
  theme(
    axis.text.x = element_text(angle = 45, vjust = 0.5, hjust = 1),
    legend.position = "none") +
  labs(title="alcohol start age",xlab="drug use frequency")
...
density_plot=
  ...
  geom_vline(data=BMI, aes(xintercept=mean(bmi), color=sex),
             linetype="dashed") + 
  facet_grid(. ~ drug_use_freq) +
  labs(title = "density plot for BMI",xlab="drug use frequency"))
